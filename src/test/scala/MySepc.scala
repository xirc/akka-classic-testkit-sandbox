  ...
  with BeforeAndAfterAll
{
  ...
  "assertion example" must {
    ...
    "expectMsgAllClassOf" in {
      ...
      val all = expectMsgAllClassOf(classOf[Int], classOf[String], classOf[Some[Int]])
      all(0) mustBe "123"
      all(1) mustBe 123
    ...
    "expectMsgAllConformingOf" in {
      ...
      echo ! 123
      echo ! Some(123)
      val all = expectMsgAllConformingOf(classOf[Int], classOf[String], classOf[Option[Int]])
      all(0) mustBe "123"
      all(1) mustBe 123
      all(2) mustBe Some(123)
    }
    ...
    "receiveWhile" in {
      ...
      val messages = receiveWhile(4.seconds, 1.5.seconds, 2) {
        case value => value
      }
    ...
    "awaitCond" in {
      ...
      akka.pattern.after(0.5.seconds)(Future.successful(1)).foreach {
        value => mutable = None
      }
      akka.pattern.after(1.seconds)(Future.successful(2)).foreach {
        value => mutable = Some(value)
      }
